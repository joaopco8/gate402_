...
function RoleCard({ role }: { role: Role }) {
  ...
  return (
    <div
      style={{ border: `1px solid ${hov || open ? `${role.accent}40` : '#2A2E2A'}`, borderRadius: 10, overflow: 'hidden', transition: 'border-color 250ms ease' }}
      onMouseEnter={() => setHov(true)}
      onMouseLeave={() => setHov(false)}
    >
      <button
        onClick={() => setOpen(v => !v)}
        style={{ width: '100%', background: 'none', border: 'none', padding: '28px 32px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, textAlign: 'left' }}
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 10, fontFamily: MONO, letterSpacing: '0.1em', textTransform: 'uppercase', color: role.accent, border: `1px solid ${role.accent}44`, borderRadius: 4, padding: '2px 8px' }}>{role.tag}</span>
            <span style={{ fontSize: 12, fontFamily: MONO, color: DIM }}>{role.type} · {role.location}</span>
          </div>
          <h3 style={{ fontSize: 20, fontWeight: 300, letterSpacing: '-0.03em', color: TEXT, margin: 0, fontFamily: SANS }}>{role.title}</h3>
        </div>
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke={MUTED} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"
          style={{ flexShrink: 0, transform: open ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 250ms ease' }}>
          <path d="M3 6l5 5 5-5" />
        </svg>
      </button>
      ...
      {open && (
        <div style={{ padding: '0 32px 32px', display: 'flex', flexDirection: 'column', gap: 28, borderTop: LINE }}>
          <p style={{ fontSize: 14, color: MUTED, lineHeight: 1.8, fontWeight: 300, fontFamily: SANS, margin: '24px 0 0' }}>{role.description}</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
            <div>
              <span style={{ fontSize: 10, fontFamily: MONO, letterSpacing: '0.1em', textTransform: 'uppercase', color: DIM, display: 'block', marginBottom: 16 }}>Responsibilities</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              <div>
                <span style={{ fontSize: 10, fontFamily: MONO, letterSpacing: '0.1em', textTransform: 'uppercase', color: DIM, display: 'block', marginBottom: 16 }}>Requirements</span>
                <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div>
                <span style={{ fontSize: 10, fontFamily: MONO, letterSpacing: '0.1em', textTransform: 'uppercase', color: DIM, display: 'block', marginBottom: 16 }}>Nice to have</span>
                <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {role.nice.map((r, i) => (
                    <li key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
    ...
    <div style={{ background: '#1B1E1B', minHeight: '100vh', fontFamily: SANS, color: TEXT }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', borderLeft: LINE, borderRight: LINE }}>
        ...
        <div style={{ padding: '80px 64px 56px', borderBottom: LINE }}>
          <span style={{ fontSize: 10, fontFamily: MONO, letterSpacing: '0.12em', textTransform: 'uppercase', color: GREEN, display: 'block', marginBottom: 20 }}>careers</span>
          <h1 style={{ fontSize: 'clamp(2rem, 4vw, 3.5rem)', fontWeight: 300, letterSpacing: '-0.04em', color: '#FFFFFF', margin: '0 0 20px', lineHeight: 1.05, fontFamily: SANS }}>Join Metera.</h1>
          <p style={{ fontSize: 15, color: MUTED, margin: '0 0 40px', fontWeight: 300, maxWidth: 480 }}>
            We're building the payment layer for autonomous AI. Small team, hard problems, real impact. Remote-first.
          </p>
        ...
        <div style={{ padding: '56px 64px 80px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          <span style={{ fontSize: 10, fontFamily: MONO, letterSpacing: '0.1em', textTransform: 'uppercase', color: DIM, marginBottom: 8 }}>Open positions</span>
          {ROLES.map(role => <RoleCard key={role.title} role={role} />)}

          <div style={{ marginTop: 24, padding: '32px', border: LINE, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24, flexWrap: 'wrap' }}>
            <div>
