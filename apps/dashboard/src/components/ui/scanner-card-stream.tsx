...
const ScannerCardStream = ({
  ...
  useEffect(() => {
    ...
    type ScannerParticle = { x: number; y: number; vx: number; vy: number; radius: number; alpha: number; life: number; decay: number };
    ...
    const handleWheel = (e: WheelEvent) => {
      e.preventDefault();
      cardStreamState.current.velocity = Math.min(cardStreamState.current.velocity + Math.abs(e.deltaY) * 0.5, 800);
  ...
  return (
    <div className="relative w-screen h-screen flex items-start justify-center overflow-hidden" style={{ paddingTop: '8%' }}>
      ...
      <div
        className={`scanner-line absolute top-1/2 left-1/2 h-[280px] w-0.5 -translate-x-1/2 -translate-y-1/2 bg-gradient-to-b from-transparent via-violet-500 to-transparent rounded-full transition-opacity duration-300 z-20 pointer-events-none animate-scan-pulse ${isScanning ? 'opacity-100' : 'opacity-0'}`}
        style={{ boxShadow: `0 0 10px #a78bfa, 0 0 20px #a78bfa, 0 0 30px #8b5cf6, 0 0 50px #6366f1` }}
      />

      <div className="absolute w-screen h-[250px] flex items-center z-[5]">
        <div
          ref={cardLineRef}
        ...
        >
          {cards.map(card => (
            <div key={card.id} className="card-wrapper relative w-[400px] h-[250px] shrink-0">
              <div className="card-normal card absolute top-0 left-0 w-full h-full overflow-hidden bg-transparent shadow-[0_15px_40px_rgba(0,0,0,0.4)] z-[2] [clip-path:inset(0_0_0_var(--clip-right,0%))]">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={card.image} alt="" className="w-full h-full object-cover transition-all duration-300 ease-in-out brightness-110 contrast-110" />
              </div>
              <div className="card-ascii card absolute top-0 left-0 w-full h-full overflow-hidden bg-transparent z-[1] [clip-path:inset(0_calc(100%-var(--clip-left,0%))_0_0)]">
