      ...
      <nav style={{ padding: '4px 0' }}>
        {NAV_GROUPS.map(group => (
          <div key={group.group} style={{ marginBottom: 4 }}>
            <div style={{ padding: '6px 14px', fontFamily: T.font, fontSize: 11, fontWeight: 600, color: T.textMuted, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
              {group.group}
            </div>
            {group.items.map(item => {
              ...
              return (
                <button key={item.id} onClick={() => scrollTo(item.id)} style={{
                  ...
                  padding: '5px 14px 5px 18px', fontFamily: T.font, fontSize: 13,
...
const H2 = ({ id, children }: { id: string; children: React.ReactNode }) => (
  <h2 id={id} style={{ fontFamily: T.font, fontWeight: 600, fontSize: 20, color: T.textPrimary, borderBottom: `1px solid ${T.border}`, paddingBottom: 10, marginTop: 56, marginBottom: 18, scrollMarginTop: T.navH + 16, lineHeight: 1.2 }}>
    {children}
  </h2>
)
const H3 = ({ id, children }: { id?: string; children: React.ReactNode }) => (
  <h3 id={id} style={{ fontFamily: T.font, fontWeight: 600, fontSize: 14, color: T.textPrimary, marginTop: 24, marginBottom: 6, scrollMarginTop: T.navH + 16 }}>
    {children}
  </h3>
)
